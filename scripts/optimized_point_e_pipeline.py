...
class OptimizedPointEGenerator:
    ...
    def __init__(
        ...
        output_dir: Path = Path("results/optimized_point_cloud_outputs"),
        ...
        if num_workers is None:
            ...
            logger.warning(f"Invalid num_workers {num_workers}, setting to 1")
            num_workers = 1
    ...
    def generate_point_clouds_parallel(
        ...
        # Validate inputs
        if not prompts:
            logger.info("Empty prompt list provided, returning empty results")
            return {}
        ...
        for prompt in prompts:
            if not isinstance(prompt, str) or not prompt.strip():
                raise ValueError(f"Invalid prompt: {prompt}. All prompts must be non-empty strings.")
        ...
        # Validate batch_size
        if batch_size < 1:
            logger.warning(f"Invalid batch_size {batch_size}, setting to 1")
            batch_size = 1
        elif batch_size > 5:  # Limit batch size for stability
            logger.warning(f"Large batch_size {batch_size} may cause instability, limiting to 5")
            batch_size = 5
    ...
    def save_results(
        ...
        # Validate inputs
        if not point_clouds:
            logger.info("No point clouds to save")
            return []

        if not isinstance(point_clouds, dict):
            raise ValueError("point_clouds must be a dictionary")
        ...
        for prompt, pc in point_clouds.items():
            ...
            if pc is None:
                logger.warning(f"Skipping None point cloud for prompt: {prompt}")
                continue

            if not hasattr(pc, 'coords') or pc.coords is None:
                logger.warning(f"Point cloud for prompt '{prompt}' has no coordinates")
                continue

            if len(pc.coords) == 0:
                logger.warning(f"Point cloud for prompt '{prompt}' is empty")
                continue

            if not np.isfinite(pc.coords).all():
                logger.warning(f"Point cloud for prompt '{prompt}' contains invalid values (NaN/Inf)")
                continue
